     ...
     "authors": [
       {
         "id": "<autor-slug>",
         "name": "…",
         "avatar": "/media/authors/<autor-slug>.webp"
       }
     ],
     "status": "planned",
...
4. **Автор уже в клубе?** Проверь `authors` в `index.json`: если человек там есть
   (он автор другой книги) — возьми его `id`, `name` и `avatar` буквально, новую
   аватарку не заводи. Иначе автор раздвоится, и его книги не соберутся на одну
   страницу. Для нового автора `id` — kebab-case транслитом от имени.
5. Аватарки новых авторов → `media/authors/<autor-slug>.webp` (см. «Конвертация изображений», квадрат 400×400). Если файлов пока нет — оставь путь в `meta.json` как договорённость и предупреди, что картинку надо добавить позже.
6. Проверь: `JSON.parse` для `meta.json`, что все пути картинок резолвятся, `npx prettier --check` по новым файлам.
7. Коммит: `feat(books): добавить книгу "<Название>"`. Не пушить (только по явной просьбе).
